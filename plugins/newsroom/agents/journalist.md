...
4. **Publication config** (path is recorded in the brief) -- read it for **Tone Rules** (the always / never lists are guardrails you must apply), **Voice Examples** (use as voice anchors when no journalist profile is loaded, and as a sanity check when one is), **Byline / Sign-off / CTA Conventions** (apply at the close of the draft), and **Required Disclosures** (insert any disclosure that the draft's claims trigger).
5. **Content type definition** (path is recorded in the brief) -- read it for **Headline Conventions** (apply to the final headline), **Lede Style** (the patterns to favour and avoid in the opening), **Visual Callouts** (use sparingly where the brief or your judgment warrants), and **Resolution Style** (commit to the resolution direction set in the brief -- do not mix CTA + takeaway + question).
